...
#
# Usage: scripts/ci/build-cli.sh [TARGET]
...
# (scripts/ci/package-tooling.sh) publishes these standalone and bundles each
# platform's CLI + language server into a wavelet-<target>.tar.gz for the
# Homebrew formula.
#
# Used by the `Release` GitHub workflow alongside scripts/ci/build-lsp.sh (the
