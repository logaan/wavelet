...
| variant — case shadowing a builtin name (backend generic path) | yes | yes | yes | **diverges silently** | **GAP (item 3, in flight — no PR open as of 2026-07-31)** — `src/emit/call.rs:297` consults `BUILTINS` before `local_cases` (`:303`), the opposite precedence from the interpreter and checker. Repro from the step Thing: after `DefType t [not(u32) other]`, `to-string(not(5))` is `"not(5)"` interpreted but `"false"` compiled. |
...
2. **item 3 (in flight)** — backend generic call path resolves builtins before
   local variant cases (silent divergence).
